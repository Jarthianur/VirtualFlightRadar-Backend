...
class is_iterable
{
    ...
    static auto test(int) -> decltype(std::begin(std::declval<TT&>()) != std::end(std::declval<TT&>()),
...
template<typename T>
static testsuite_shared describeParallel(const char* name, runner& runner = runner::default_instance())
{
...
protected:
    void report_ts(const testsuite_shared ts) override
    {
        ...
        *this << SCTF_SPACE << "<testsuite id=\"" << m_id++ << "\" name=\"" << ts->name() << "\" errors=\""
              << ts->statistics().errors() << "\" tests=\"" << ts->statistics().tests() << "\" failures=\""
              << ts->statistics().failures() << "\" skipped=\"0\" time=\"" << ts->time() << "\" timestamp=\""
              << buff << "\">" << SCTF_LF;
    ...
    {
        *this << SCTF_XSPACE << "<testcase name=\"" << tc.name() << "\" classname=\"" << tc.context()
              << "\" time=\"" << tc.duration() << "\"";
        switch (tc.state())
        {
            case _::testcase::result::ERROR:
                *this << ">" << SCTF_LF << SCTF_XSPACE << SCTF_SPACE << "<error message=\"" << tc.err_msg()
                      << "\"></error>" << SCTF_LF << SCTF_XSPACE << "</testcase>";
                break;
            case _::testcase::result::FAILED:
                *this << ">" << SCTF_LF << SCTF_XSPACE << SCTF_SPACE << "<failure message=\"" << tc.err_msg()
                      << "\"></failure>" << SCTF_LF << SCTF_XSPACE << "</testcase>";
                break;
    ...
    {
        *this << "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>" << SCTF_LF << "<testsuites>" << SCTF_LF;
    }
...
protected:
    ...
    void end_report() override
    {
        ...
        *this << "Result:: passed: " << m_abs_tests - m_abs_fails - m_abs_errs << "/" << m_abs_tests
              ...
              << m_abs_tests << " ; time = " << m_abs_time << "ms" << (m_color ? ANSI_RESET : "") << SCTF_LF;
    }
    bool m_color;
    bool m_out;
};
static reporter_shared createPlainTextReporter(std::ostream& stream, bool color = false, bool out = false)
{
...
static reporter_shared createPlainTextReporter(const char* file, bool color = false, bool out = false)
{
...
protected:
    ...
    {
        *this << "<h3>" << ts->name() << "</h3>"
              << "<p>Tests: " << ts->statistics().tests() << " Failures: " << ts->statistics().failures()
              << " Errors: " << ts->statistics().errors() << " Time: " << ts->time() << "ms</p><table><thead>"
              << TR << TH << "Name" << TH_ << TH << "Context" << TH_ << TH << "Time" << TH_ << TH << "Status"
              << TH_ << TR_ << "</thead><tbody>";
        abstract_reporter::report_ts(ts);
        *this << "</tbody></table>";
    }
    void report_tc(const _::testcase& tc) override
    {
        ...
        *this << "<tr class=\"" << status << "\">" << TD << tc.name() << TD_ << TD << tc.context() << TD_
              << TD << tc.duration() << "ms" << TD_ << TD << status << TD_ << TR_;
    }
    void begin_report() override
    {
    ...
    void end_report() override
    {
        *this << "<footer><h3>Summary</h3><p>Tests: " << m_abs_tests << " Failures: " << m_abs_fails
              << " Errors: " << m_abs_errs << " Time: " << m_abs_time << "ms</p></footer></body></html>";
    }
};
...
#    define COMPARATOR(NAME, COMPSTR, PRED)                                                             \
        ...
        {                                                                                               \
            return (PRED) ? success : comparison(NAME##_comp_str, to_string(value), to_string(expect)); \
        }                                                                                               \
        }                                                                                               \
        }
...
{
    return value != expect ? success : comparison(unequals_comp_str, to_string(value), to_string(expect));
}
template<typename V, typename E = V,
         ENABLE_IF(NOT IS_FLOAT(V) AND NOT IS_UNEQUAL_COMPARABLE(V, E) AND IS_EQUAL_COMPARABLE(V, E))>
static comparison unequals(const V& value, const E& expect)
{
    return value == expect ? comparison(unequals_comp_str, to_string(value), to_string(expect)) : success;
}
...
{
    return value == expect ? success : comparison(equals_comp_str, to_string(value), to_string(expect));
}
template<typename V, typename E = V,
         ENABLE_IF(NOT IS_FLOAT(V) AND NOT IS_EQUAL_COMPARABLE(V, E) AND IS_UNEQUAL_COMPARABLE(V, E))>
static comparison equals(const V& value, const E& expect)
{
    return value != expect ? comparison(equals_comp_str, to_string(value), to_string(expect)) : success;
}
template<typename V, typename E = V, ENABLE_IF(IS_FLOAT(V) AND IS_FLOAT(E))>
...
#    define assertZero(VALUE)                                                                              \
        sctf::_::_assertStatement(VALUE, static_cast<decltype(VALUE)>(0), sctf::EQUALS<decltype(VALUE)>(), \
                                  sctf::_::location{__FILE__, __LINE__})
...
{
template<typename V, typename E = V>
static void _assertStatement(const V& value, const E& expect, comparator<V, E> comp, const location& loc)
{
...
static void _assertNoExcept(const test_function& func, const location& loc)
{
    ...
    {
        throw assertion_failure(std::string("Expected no exception, caught '") + to_string(e) + "'", loc);
    }
    catch (...)
    {
